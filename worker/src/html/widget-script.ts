  ...
  return `(function() {
    ...
    const ChatInterface = {
        build() {
            ...
            frame.style.cssText = 'position:fixed;bottom:28px;' + alignment + ';width:420px;max-width:calc(100vw - 56px);height:640px;max-height:calc(100vh - 120px);background:#111111;border-radius:20px;box-shadow:0 12px 48px rgba(0,0,0,0.6);display:none;flex-direction:column;overflow:hidden;z-index:9999998;font-family:-apple-system,BlinkMacSystemFont,\\'Segoe UI\\',Roboto,\\'Helvetica Neue\\',Arial,sans-serif;';

            frame.innerHTML = '<header style="background:' + cfg.primary_color + ';color:#fff;padding:20px;display:flex;align-items:center;justify-content:space-between;border-radius:20px 20px 0 0"><div style="display:flex;align-items:center;gap:12px">' + (cfg.bot_avatar_url ? '<img src="' + cfg.bot_avatar_url + '" alt="Bot avatar" style="width:36px;height:36px;border-radius:50%;border:2px solid rgba(255,255,255,0.3);object-fit:cover"/>' : '<div style="width:36px;height:36px;border-radius:50%;background:rgba(255,255,255,0.2);display:flex;align-items:center;justify-content:center;font-weight:bold">' + cfg.bot_name[0] + '</div>') + '<div><div style="font-weight:600;font-size:16px;letter-spacing:-0.2px">' + cfg.bot_name + '</div><div style="font-size:12px;opacity:0.9;margin-top:2px">🟢 Active now</div></div></div><button id="ib-close-btn" aria-label="Close chat" style="background:rgba(255,255,255,0.2);border:none;color:#fff;cursor:pointer;padding:4px;display:flex;align-items:center;justify-content:center"><svg width="20" height="20" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6L6 18M6 6l12 12"/></svg></button></header><div id="ib-messages" style="flex:1;overflow-y:auto;padding:20px;display:flex;flex-direction:column;gap:14px;background:#0a0a0a"></div><footer style="padding:18px;background:#111111;border-top:1px solid rgba(0,245,255,0.2)"><form id="ib-input-form" style="display:flex;gap:10px;align-items:flex-end"><input type="text" id="ib-text-input" placeholder="Ask me anything..." aria-label="Message input" style="flex:1;padding:14px 16px;border:1px solid rgba(0,245,255,0.25);border-radius:12px;font-size:15px;outline:none;font-family:inherit;transition:border-color 0.2s;background:#1a1a1a;color:#e2e8f0"/><button type="submit" id="ib-send-btn" aria-label="Send message" style="background:' + cfg.primary_color + ';color:white;border:none;border-radius:12px;padding:14px 24px;cursor:pointer;font-weight:700;font-size:15px;transition:opacity 0.2s;min-width:80px" onmouseover="if(!this.disabled) this.style.opacity=\\'0.85\\'" onmouseout="this.style.opacity=\\'1\\'">Send</button></form></footer>';

            // Inject custom styles
            const styleSheet = document.createElement("style");
            styleSheet.textContent = '#ib-messages::-webkit-scrollbar { width: 8px; }#ib-messages::-webkit-scrollbar-track { background: transparent; }#ib-messages::-webkit-scrollbar-thumb { background: rgba(0,245,255,0.25); border-radius: 4px; }#ib-messages::-webkit-scrollbar-thumb:hover { background: rgba(0,245,255,0.45); }.ib-msg { display: flex; gap: 10px; max-width: 85%; animation: slideIn 0.3s ease-out; }.ib-msg-user { margin-left: auto; flex-direction: row-reverse; }.ib-msg-text { background: #1e1e1e; color: #e2e8f0; padding: 12px 16px; border-radius: 16px; font-size: 15px; line-height: 1.5; box-shadow: 0 2px 8px rgba(0,0,0,0.4); word-wrap: break-word; }.ib-msg-user .ib-msg-text { background: ' + cfg.primary_color + '; color: #fff; border-bottom-right-radius: 4px; }.ib-msg-assistant .ib-msg-text { border-bottom-left-radius: 4px; }#ib-text-input:focus { border-color: ' + cfg.primary_color + '; background: #1a1a1a; }#ib-text-input::placeholder { color: #64748b; }#ib-send-btn:disabled { opacity: 0.5; cursor: not-allowed; }@keyframes slideIn { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } }';
